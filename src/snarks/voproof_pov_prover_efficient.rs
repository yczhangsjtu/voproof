...
pub struct POVProverEfficientProof<E: PairingEngine> {
  pub cm_a_vec: Commitment<E>,
  pub cm_b_vec: Commitment<E>,
...
impl<E: PairingEngine> SNARK<E> for VOProofPOVProverEfficient {
  ...
  fn prove(pk: &Self::PK, x: &Self::Ins, w: &Self::Wit) -> Result<Self::Pf, Error> {
    ...
    define!(n, 3 * cap_c);
    redefine_zero_pad_concat_vector!(a_vec, n, delta_vec);
    define_commit_vector!(cm_a_vec, a_vec, pk.powers, n + 1);
    redefine_zero_pad_concat_vector!(b_vec, n, delta_vec_1);
    define_commit_vector!(cm_b_vec, b_vec, pk.powers, n + 1);
    redefine_zero_pad_concat_vector!(c_vec, n, delta_vec_2);
    define_commit_vector!(cm_c_vec, c_vec, pk.powers, n + 1);
    ...
    define_accumulate_vector_mul!(
      ...
      i,
      mul!(
        vector_index!(a_vec, minus_i64!(i, 1))
          + vector_index!(b_vec, minus_i64!(i, cap_c + 1))
          + vector_index!(c_vec, minus_i64!(i, 2 * cap_c + 1))
    ...
    redefine_zero_pad_concat_vector!(r_vec, n, delta_vec_3);
      ...
      vector_concat!(
        ...
        expression_vector!(
          ...
          c * vector_index!(t_vec, minus_i64!(i + n, 1))
            * (vector_index!(a_vec, minus_i64!(i + n, 1))
              + vector_index!(b_vec, minus_i64!(i + n, cap_c + 1))
              + vector_index!(c_vec, minus_i64!(i + n, 2 * cap_c + 1))
              - vector_index!(x_vec, minus_i64!(i + n, 1)))
            + c_2
              * vector_index!(r_vec, minus_i64!(i + n, -3 * cap_c + n + 1))
              * (vector_index!(a_vec, minus_i64!(i + n, -3 * cap_c + n + 1))
                + vector_index!(b_vec, minus_i64!(i + n, -2 * cap_c + n + 1))
                + vector_index!(c_vec, minus_i64!(i + n, -cap_c + n + 1))
                + range_index!(1, 3 * cap_c, minus_i64!(i + n, -3 * cap_c + n + 1)) * zeta_1
                + vector_index!(pk.sigma_vec, minus_i64!(i + n, -3 * cap_c + n + 1)) * zeta)
            - vector_index!(c_vec, minus_i64!(i + n, -cap_c_m + n + 1))
              * range_index!(1, cap_c_m, minus_i64!(i + n, -cap_c_m + n + 1))
            + vector_index!(a_vec, minus_i64!(i + n, -cap_c_m + n + 1))
              * vector_index!(b_vec, minus_i64!(i + n, -cap_c_m + n + 1))
    ...
    define_vector_domain_evaluations_dict!(_a_vec_left_eval_dict, _a_vec_right_eval_dict);
    define_vector_domain_evaluations_dict!(_b_vec_left_eval_dict, _b_vec_right_eval_dict);
    define_vector_poly_mul_shift!(
      ...
      omega,
      shiftlength,
      _a_vec_left_eval_dict,
      _b_vec_right_eval_dict
    );
    ...
    define_vector_power_mul!(v_vec_11, c_vec, c_3, cap_c_m);
    define_vector_power_mul!(v_vec_12, c_vec, c_3, cap_c_a);
    define_vector_power_mul!(v_vec_13, a_vec, c_3, cap_c_a);
    define_vector_power_mul!(v_vec_14, b_vec, c_3, cap_c_a);
    define_vector_power_mul!(v_vec_15, v_vec_9, gamma, 3 * cap_c);
    define_vector_power_mul!(v_vec_16, v_vec_9, one!(), 3 * cap_c);
    define_vector_power_mul!(v_vec_17, t_vec_1, c_3, 3 * cap_c - cap_c_m + 1);
    ...
    define!(c_12, -alpha * power(omega, cap_c_a + cap_c_m - 1));
    define!(c_13, alpha * power(omega, cap_c_a + cap_c_m - 1));
    define!(c_14, power(alpha, 3) * omega * zeta);
    define!(c_15, power(alpha, 3) * omega * zeta_1);
    define!(c_16, -power(alpha, 3) * zeta_1);
    define!(c_17, -power(omega, 6 * cap_c - cap_c_m));
    define_expression_vector!(
      ...
      -power(alpha, 4) * power(omega, 3 * cap_c - 1) * delta!(i - maxshift - n, 1)
        ...
        + c
          * vector_index!(
            v_vec_3,
            minus_i64!(i - maxshift - n, cap_c - shiftlength_2 + 1)
          )
        + c
          * vector_index!(
            v_vec_4,
            minus_i64!(i - maxshift - n, 2 * cap_c - shiftlength_3 + 1)
          )
        + c_1 * vector_index!(c_vec, minus_i64!(i - maxshift - n, 2 * cap_c + 1))
        + c_1 * vector_index!(a_vec, minus_i64!(i - maxshift - n, 1))
        + c_1 * vector_index!(b_vec, minus_i64!(i - maxshift - n, cap_c + 1))
        + c_10 * range_index!(1, 3 * cap_c, minus_i64!(i - maxshift - n, 1))
        + c_11 * vector_index!(v_vec_11, minus_i64!(i - maxshift - n, 2 - cap_c_m))
        + c_12
          * vector_index!(
            v_vec_12,
        ...
        + c_14 * vector_index!(v_vec_15, minus_i64!(i - maxshift - n, -shiftlength_8))
        + c_15 * vector_index!(v_vec_16, minus_i64!(i - maxshift - n, -shiftlength_8))
        + c_16 * vector_index!(v_vec_16, minus_i64!(i - maxshift - n, 1 - shiftlength_8))
        + c_17 * vector_index!(v_vec_17, minus_i64!(i - maxshift - n, -3 * cap_c + cap_c_m))
        + c_2 * vector_index!(v_vec_6, minus_i64!(i - maxshift - n, 1 - shiftlength_5))
        + c_2
          * vector_index!(
            v_vec_7,
            minus_i64!(i - maxshift - n, cap_c - shiftlength_6 + 1)
          )
        + c_2
          * vector_index!(
            v_vec_8,
            minus_i64!(i - maxshift - n, 2 * cap_c - shiftlength_7 + 1)
          )
        + c_4 * vector_index!(v_vec_1, minus_i64!(i - maxshift - n, 1 - shiftlength))
        + c_5 * vector_index!(v_vec_5, minus_i64!(i - maxshift - n, 1 - shiftlength_4))
        + c_6 * vector_index!(v_vec_6, minus_i64!(i - maxshift - n, -shiftlength_5))
        + c_6 * vector_index!(v_vec_7, minus_i64!(i - maxshift - n, cap_c - shiftlength_6))
        + c_6
          * vector_index!(
            v_vec_8,
            minus_i64!(i - maxshift - n, 2 * cap_c - shiftlength_7)
          )
        + c_7
          * vector_index!(
            v_vec_9,
            minus_i64!(i - maxshift - n, 3 * cap_c - shiftlength_8)
          )
        + c_8 * vector_index!(v_vec_10, minus_i64!(i - maxshift - n, 1 - shiftlength_9))
    ...
    define_expression_vector!(
      ...
      -power(alpha, 4) * power(omega, 3 * cap_c - 1) * delta!(i + 1, 1)
        ...
        + c * vector_index!(v_vec_3, minus_i64!(i + 1, cap_c - shiftlength_2 + 1))
        + c * vector_index!(v_vec_4, minus_i64!(i + 1, 2 * cap_c - shiftlength_3 + 1))
        + c_1 * vector_index!(c_vec, minus_i64!(i + 1, 2 * cap_c + 1))
        + c_1 * vector_index!(a_vec, minus_i64!(i + 1, 1))
        + c_1 * vector_index!(b_vec, minus_i64!(i + 1, cap_c + 1))
        + c_10 * range_index!(1, 3 * cap_c, minus_i64!(i + 1, 1))
        + c_11 * vector_index!(v_vec_11, minus_i64!(i + 1, 2 - cap_c_m))
        + c_12 * vector_index!(v_vec_12, minus_i64!(i + 1, -cap_c_a - cap_c_m + 2))
        + c_13 * vector_index!(v_vec_13, minus_i64!(i + 1, -cap_c_a - cap_c_m + 2))
        + c_13 * vector_index!(v_vec_14, minus_i64!(i + 1, -cap_c_a - cap_c_m + 2))
        + c_14 * vector_index!(v_vec_15, minus_i64!(i + 1, -shiftlength_8))
        + c_15 * vector_index!(v_vec_16, minus_i64!(i + 1, -shiftlength_8))
        + c_16 * vector_index!(v_vec_16, minus_i64!(i + 1, 1 - shiftlength_8))
        + c_17 * vector_index!(v_vec_17, minus_i64!(i + 1, -3 * cap_c + cap_c_m))
        + c_2 * vector_index!(v_vec_6, minus_i64!(i + 1, 1 - shiftlength_5))
        + c_2 * vector_index!(v_vec_7, minus_i64!(i + 1, cap_c - shiftlength_6 + 1))
        + c_2 * vector_index!(v_vec_8, minus_i64!(i + 1, 2 * cap_c - shiftlength_7 + 1))
        + c_4 * vector_index!(v_vec_1, minus_i64!(i + 1, 1 - shiftlength))
        + c_5 * vector_index!(v_vec_5, minus_i64!(i + 1, 1 - shiftlength_4))
        + c_6 * vector_index!(v_vec_6, minus_i64!(i + 1, -shiftlength_5))
        + c_6 * vector_index!(v_vec_7, minus_i64!(i + 1, cap_c - shiftlength_6))
        + c_6 * vector_index!(v_vec_8, minus_i64!(i + 1, 2 * cap_c - shiftlength_7))
        + c_7 * vector_index!(v_vec_9, minus_i64!(i + 1, 3 * cap_c - shiftlength_8))
        + c_8 * vector_index!(v_vec_10, minus_i64!(i + 1, 1 - shiftlength_9))
    ...
    define_eval_vector_expression!(y, omega / z, i, vector_index!(a_vec, i), n + 1);
    ...
    define!(
      c_18,
      (-alpha * one!() * z * power(omega / z, cap_c_m) * (one!() - power(omega / z, cap_c_a))
        + (omega - one!() * z)
          * (power(alpha, 3)
            * (-one!() * y_2 * power(z, -2 * cap_c + n)
              + power(z, -2 * cap_c + n - 1) * (omega * y_2 + one!() * z))
            + power(alpha, 2) * one!() * y_1 * power(z, cap_c)
            + y * power(z, -cap_c_m + n) * power(omega / z, 3 * cap_c - cap_c_m)))
        / (omega - one!() * z)
    );
    define!(
      c_19,
      (power(alpha, 2)
        * (omega - one!() * z)
        * (alpha
          * (-one!() * y_2 * power(z, -cap_c + n)
            + power(z, -cap_c + n - 1) * (omega * y_2 + one!() * z))
          + one!() * y_1 * power(z, 2 * cap_c))
        + alpha * one!() * z * power(omega / z, cap_c_m) * (one!() - power(omega / z, cap_c_a))
        + power(z, -cap_c_m + n + 1)
    ...
    define!(
      ...
      alpha
        * one!()
        * (power(alpha, 2) * (omega - one!() * z) * (omega * y_2 + one!() * z)
          + alpha * one!() * z * (omega - one!() * z) * (-alpha * y_2 + y_1)
          - power(z, 2) * power(omega / z, cap_c_m) * (one!() - power(omega / z, cap_c_a)))
        / (z * (omega - one!() * z))
    );
    define!(
      c_21,
    ...
    define_vec_mut!(
      ...
      expression_vector!(
        ...
        linear_combination_base_zero!(
          c_18,
          vector_index!(b_vec, i),
          c_19,
          vector_index!(c_vec, i),
          c_20,
          vector_index!(a_vec, i),
          c_22,
    ...
    define_poly_from_vec!(a_vec_poly, a_vec);
    ...
    define!(fs, vec!(a_vec_poly, r_vec_poly));
  ...
  fn verify(vk: &Self::VK, x: &Self::Ins, proof: &Self::Pf) -> Result<(), Error> {
    ...
    let rng = &mut test_rng();
    let cm_a_vec = proof.cm_a_vec;
    let cm_b_vec = proof.cm_b_vec;
    ...
    define!(f_commitments, vec!(cm_a_vec, cm_r_vec));
