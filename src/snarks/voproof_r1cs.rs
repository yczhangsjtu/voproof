...
impl<E: PairingEngine> SNARK<E> for VOProofR1CS {
    ...
    fn prove(pk: &Self::PK, x: &Self::Ins, w: &Self::Wit) -> Result<Self::Pf, Error> {
        ...
        define_sum!(
          c,
          -z*(mu*(one!() - power(omega/z, 3*cap_h))*(gamma*omega - one!()*z) - (omega - one!()*z)*(one!() - power(gamma*omega/z, 3*cap_h)))/((omega - one!()*z)*(gamma*omega - one!()*z)),
          power(alpha, 5)*y_1*power(z, -3*cap_h - cap_k + n)*power(omega/z, -3*cap_h + cap_s_a + cap_s_b + cap_s_c),
          -power(alpha, 5)*beta*y*power(z, -3*cap_h - cap_k + n)*power(omega/z, cap_s_a + cap_s_b + cap_s_c));
        define_sum!(
          c_1,
          z*(one!() - power(z, 3*cap_h))*(one!() - power(omega/z, 3*cap_h))/((omega - one!()*z)*(one!() - z)),
          alpha*z*(one!() - power(z, cap_k))*(one!() - power(omega/z, cap_k))/((omega - one!()*z)*(one!() - z)),
          power(alpha, 2)*mu*nu*y*power(z, cap_k)*(one!() - power(z, ell_1))/(one!() - z),
          power(alpha, 2)*power(z, cap_k + 1)*power(omega/z, cap_k)*(one!() - power(z, ell_1))*(one!() - power(omega/z, cap_s_a + cap_s_b + cap_s_c))/((omega - one!()*z)*(one!() - z)),
          mul!(
          power(alpha, 4)*power(z, 3*cap_h + 1)*power(omega/z, 3*cap_h)*(-omega*power(omega/z, ell) + one!()*z)/(omega - one!()*z),
          ...
          i),
          ell)),
          power(alpha, 4)*power(z, 3*cap_h)*power(omega/z, 3*cap_h)*(-omega*power(omega/z, ell) + one!()*z)/(omega - one!()*z),
          power(alpha, 5)*y_2*(omega - z)*(one!() - power(z, n))/(z*(one!() - z)),
          power(alpha, 6)*y_2*power(z, n - 1));
        define_sum!(
          c_2,
          -alpha*z*(nu*(one!() - power(omega/z, cap_k))*(gamma*omega - one!()*z) - (omega - one!()*z)*(one!() - power(gamma*omega/z, cap_k)))/((omega - one!()*z)*(gamma*omega - one!()*z)));
        define_sum!(
          c_3,
          -power(alpha, 2)*mu*y*power(z, cap_k));
        define_sum!(
          c_4,
          -power(alpha, 2)*nu*y*power(z, cap_k));
        define_sum!(
          c_5,
          power(alpha, 2)*y*power(z, cap_k));
        define_sum!(
          c_6,
          power(alpha, 3)*y_1*power(z, -2*cap_h + n)*power(omega/z, -cap_h + cap_k + cap_s_a + cap_s_b + cap_s_c),
          power(alpha, 3)*power(z, -3*cap_h + n + 1)*power(omega/z, -cap_h + cap_k + cap_s_a + cap_s_b + cap_s_c)*(one!() - power(omega/z, cap_h))/(omega - one!()*z),
          power(alpha, 4)*power(omega/z, 3*cap_h)*(omega*power(omega/z, ell) - one!()*z)/(omega - one!()*z));
        define_sum!(
          c_7,
          -power(alpha, 5)*beta*y*power(z, -ell_1 + n));
        define_sum!(
          c_8,
          power(z, n)*power(omega/z, cap_k + cap_s_a + cap_s_b + cap_s_c)*(one!() - power(omega/z, cap_s_a + cap_s_b + cap_s_c + 1))/(omega - one!()*z));
        define_sum!(
          c_9,
          -power(z, -cap_d));
        define_sum!(
